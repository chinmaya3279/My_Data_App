...
## 6 Count section:
    ...
    with st.expander('Value Count'):
        ...
        if count:
            ...
            fig = px.bar(data_frame=result, x='Product', y='index', text='Product', template='plotly_white')
            st.plotly_chart(fig)
            fig = px.line(data_frame=result, x='Product', y='index', text='Product', template='plotly_white')
            st.plotly_chart(fig)
            fig = px.pie(data_frame=result, names=column , values='Product')
            st.plotly_chart(fig)
## 7 Group By:           
